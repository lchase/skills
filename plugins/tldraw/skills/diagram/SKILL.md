...
description: >-
  ...
  diagram — and especially when they want it as a tldraw file they can open and tweak, or
...
1. **`<name>.tldr`** — a real tldraw document. Opens in the tldraw editor (tldraw.com, the
   desktop app, or an embedded `<Tldraw>`), fully editable. `.tldr` is the only extension
   tldraw recognizes — `.tldraw` will not open and cannot be rendered, so the script always
   writes `.tldr` (and rewrites any other extension you pass).
2. **`<name>.png`** (and `<name>.svg` when useful) — a flat render for pasting into docs,
   PRs, chat.
...
- **`w` / `h`** (optional, default 180 × 90) — bump `w` for longer labels.
...
directory) and that they can open it at tldraw.com (File → Open) or in the tldraw editor to
rearrange anything. Don't delete the outputs.
...
straight into the node/edge spec — same workflow from step 2.

## Reference

`references/tldr-format.md` — the `.tldr` file structure, the record shapes the script
