...
- `/smart-review:pr-comments <PR number or URL>` — the reverse direction: triages a PR's *existing* unresolved review comments (Copilot, human reviewers) via GraphQL (`reviewThreads(isResolved: false)`), classifies each fix/docs/explain/disagree, gates on approval before editing code and again before pushing, then replies via REST and resolves each addressed thread via the `resolveReviewThread` mutation. Not a review pass and doesn't touch the lenses.
